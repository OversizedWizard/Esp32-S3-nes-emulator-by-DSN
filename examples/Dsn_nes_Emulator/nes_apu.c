...
void apu_process(int16 *buffer, int num_samples, bool stereo)
{
   int32 prev_sample = apu.prev_sample;
   ...
   while (num_samples--)
   {
      ...
      /* do filtering */
      if (APU_FILTER_WEIGHTED == apu.filter_type)
      {
         accum = (accum + accum + accum + prev_sample) >> 2;
      }
      else if (APU_FILTER_LOWPASS == apu.filter_type)
      {
         accum = (accum + prev_sample) >> 1;
      }
      prev_sample = accum;
      ...
      /* signed 16-bit output, stereo interleaved */
      *buffer++ = (int16)accum;
      if (stereo)
         *buffer++ = (int16)accum;
   }
...
void apu_setparams(double base_freq, int sample_rate, int refresh_rate, int sample_bits)
{
   ...
   apu.samples_per_frame = apu.num_samples;
   if (0 == base_freq)
...
apu_t *apu_create(double base_freq, int sample_rate, int refresh_rate, int sample_bits)
{
   ...
   /* Allocate APU's own stereo buffer (retro-go style) */
   /* We allocate enough space for the MAXIMUM possible sample rate (48000Hz) 
      to prevent a heap buffer overflow if the user increases the sample rate 
      via the ingame menu at runtime. (48000/50+2)*4 = ~3.8KB */
   temp_apu->buffer = (int16 *)NOFRENDO_MALLOC((48000 / 50 + 2) * 4);
   if (NULL == temp_apu->buffer)
   {
      NOFRENDO_FREE(temp_apu);
      return NULL;
   }
   temp_apu->stereo = true;
   temp_apu->ext = NULL; 
