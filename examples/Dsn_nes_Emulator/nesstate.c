...
static void get_save_filename(char *buf, const char *rom_path, int slot)
{
   const char *p = strrchr(rom_path, '/');
   if (p) p++; else p = rom_path;
   
   sprintf(buf, "/sd/Saved/%s", p);
   char ext[8];
   sprintf(ext, ".ss%d", slot);
...
int state_save(void)
{
   ...
   get_save_filename(fn, machine->rominfo->filename, state_slot);
 
   nofrendo_log_printf("[STATE] Saving to: %s\n", fn);
...
int state_load(void)
{
   ...
   nofrendo_log_printf("[STATE] Loading from: %s\n", fn);
