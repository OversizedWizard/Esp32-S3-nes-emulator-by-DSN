...
static int rom_loadrom(rominfo_t *rominfo)
{
   ...
   bool psram_ready = (heap_caps_get_total_size(MALLOC_CAP_SPIRAM) > 0);
   log_printf("Allocating PRG: %d bytes (PSRAM: %s)\n", rom_len, psram_ready ? "Yes" : "No");

   rominfo->rom = (uint8 *)heap_caps_malloc(rom_len, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
   if (NULL == rominfo->rom) {
      log_printf("SPIRAM PRG alloc failed, trying regular heap...\n");
      rominfo->rom = malloc(rom_len);
   }

   if (NULL == rominfo->rom)
   {
      log_printf("FATAL: Could not allocate PRG ROM image\n");
